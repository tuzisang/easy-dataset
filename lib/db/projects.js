  ...
  try {
    ...
    if (userId) {
      ...
      if (user && user.role === 'admin') {
        ...
        // Non-admin: filter by ProjectAccess and capture roles
        const accessibleProjects = await db.projectAccess.findMany({
          where: { userId },
          select: { projectId: true, role: true }
        });
        projectRoles = Object.fromEntries(accessibleProjects.map(pa => [pa.projectId, pa.role]));
        const ids = accessibleProjects.map(pa => pa.projectId);
        where = { id: { in: ids } };
      }
    }
    ...
    return projects.map(project => ({
      ...
      userRole: isAdmin ? 'admin' : projectRoles[project.id] || null
    }));
  } catch (error) {
